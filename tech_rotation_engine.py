"""
Technology industry rotation vs XLK (relative strength heatmap).

Each industry row is an **equal-weight basket** of the listed proxies (stocks or ETFs),
rebased to 1 on the first aligned date, then averaged; RS vs XLK is that composite divided
by XLK rebased the same way. Uses dividend-adjusted prices from ``data_loader.get_price_histories_long``.
"""
...
# (display name, tuple of FMP symbols — equal-weight composite vs XLK)
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("AI Compute / GPUs", ("NVDA", "AMD")),
    ("Semiconductor Equipment", ("ASML", "AMAT", "LRCX", "KLAC")),
    ("Memory", ("MU",)),
    ("Analog / Industrial Semis", ("TXN", "ADI", "ON")),
    ("Networking / AI Infrastructure", ("AVGO", "ANET", "MRVL")),
    ("Cybersecurity", ("CRWD", "PANW", "ZS", "FTNT")),
    ("Enterprise Software", ("MSFT", "CRM", "NOW", "ORCL")),
    ("Cloud / Data Infrastructure", ("AMZN", "MSFT", "GOOGL")),
    ("Data Center Power & Cooling", ("VRT", "ETN", "NVT")),
    ("Internet Platforms", ("META", "GOOGL")),
    ("Robotics & Automation", ("BOTZ",)),
    ("Equal-weight Semiconductors", ("XSD",)),
    ("Mega-cap Semiconductors", ("SMH",)),
)
...
def get_tech_rotation_prices(
    ...
    *,
    ...
    """
    Long-format dividend-adjusted closes for XLK and all group proxy symbols.
...
def _composite_rs_vs_benchmark(
    ...
    """
    Equal-weight basket vs benchmark: rebase each series to 1 on the first row where
    all basket members and the benchmark are non-null; composite = mean(rebased basket);
    RS = composite / rebased benchmark.
    """
...
def calculate_relative_strength_metrics(price_df: pd.DataFrame) -> pd.DataFrame:
    """
    RS ratio = equal-weight group composite / XLK (both rebased); horizon RS % and RS vs DMAs.
...
def _build_rs_ratio_history(price_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for industry, tickers in INDUSTRY_GROUPS:
        ...
        if rs.empty:
            continue
        name = _group_column_slug(industry, tickers)
        cols[name] = rs
    if not cols:
        return pd.DataFrame()
...
def build_tech_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    try:
        prices = get_tech_rotation_prices(session, api_key, force_refresh=force_refresh)
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLK or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLK).",
