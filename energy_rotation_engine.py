"""
Energy industry rotation vs XLE (relative strength heatmap).

Each row is an **equal-weight basket** of listed proxies (stocks or ETFs), rebased to 1 on the
first aligned date, then averaged; RS vs XLE is that composite divided by XLE rebased the same way.
...
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("Integrated Oil Majors", ("XOM", "CVX", "SHEL", "BP")),
    ("E&P / Upstream Producers", ("EOG", "FANG", "DVN", "APA")),
    ("Oilfield Services", ("SLB", "HAL", "BKR")),
    ("Refiners", ("MPC", "VLO", "PSX")),
    ("Midstream Pipelines", ("WMB", "KMI", "TRGP", "ET")),
    ("LNG & Natural Gas Export", ("LNG", "NEXT", "FLNG")),
    ("Natural Gas Producers", ("EQT", "RRC", "AR")),
    ("Offshore Drilling", ("RIG", "VAL", "NE")),
    ("Energy Equipment & Infrastructure", ("NOV", "FTI", "CHX")),
    ("Clean Energy Transition (Optional)", ("FSLR", "NXT", "BE")),
    ("Equal-Weight Energy Breadth", ("XOP",)),
)
...
def get_energy_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLE and all group proxy symbols."""
    end = date.today()
...
def build_energy_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    try:
        prices = get_energy_rotation_prices(session, api_key, force_refresh=force_refresh)
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLE or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLE).",
