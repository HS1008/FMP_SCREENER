"""
Consumer Defensive industry rotation vs XLP (relative strength heatmap).

Each row is an **equal-weight basket** of listed proxies (stocks or ETFs), rebased to 1 on the
first aligned date, then averaged; RS vs XLP is that composite divided by XLP rebased the same way.
...
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("Beverage Giants", ("KO", "PEP", "MNST")),
    ("Tobacco & Nicotine", ("PM", "MO", "BTI")),
    ("Household & Personal Care", ("PG", "CL", "KMB")),
    ("Discount & Value Retail", ("WMT", "COST", "DG")),
    ("Packaged Foods", ("GIS", "KHC", "CPB", "HSY")),
    ("Grocery & Food Retail", ("KR", "SFM", "ACI")),
    ("Beauty & Premium Staples", ("ELF", "ULTA", "EL")),
    ("Alcohol Producers", ("DEO", "STZ", "TAP")),
    ("Convenience & Fuel Retail", ("CASY", "MUSA")),
    ("Staples Breadth Proxy", ("RSPS",)),
)
...
def get_consumer_defensive_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLP and all group proxy symbols."""
    end = date.today()
...
def build_consumer_defensive_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLP or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLP).",
