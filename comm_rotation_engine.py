"""
Communication Services industry rotation vs XLC (relative strength heatmap).

Each row is an **equal-weight basket** of listed proxies (stocks or ETFs), rebased to 1 on the
first aligned date, then averaged; RS vs XLC is that composite divided by XLC rebased the same way.
...
# (display name, tuple of FMP symbols — equal-weight composite vs XLC)
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("Mega-Cap Internet Platforms", ("META", "GOOGL")),
    ("Streaming & Digital Media", ("NFLX", "DIS", "WBD", "PARA")),
    ("Telecom / Wireless", ("T", "VZ", "TMUS")),
    ("Ad Tech / Digital Advertising", ("TTD", "APP", "PINS", "SNAP")),
    ("Gaming & Interactive Entertainment", ("EA", "TTWO", "RBLX")),
    ("Music & Audio Platforms", ("SPOT", "SIRI")),
    ("Social / Creator Platforms", ("META", "SNAP", "PINS")),
    ("Travel & Booking Platforms", ("BKNG", "ABNB", "EXPE")),
    ("Chinese Internet ADRs (Optional)", ("BABA", "BIDU", "TME")),
    ("Broad Internet Breadth", ("FDN",)),
)
...
def get_comm_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLC and all group proxy symbols."""
    end = date.today()
...
def build_comm_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    try:
        prices = get_comm_rotation_prices(session, api_key, force_refresh=force_refresh)
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLC or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLC).",
