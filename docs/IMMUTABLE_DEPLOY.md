...
Verify sources `/etc/fmp/fmp-dashboard.env` only (`FMP_IDENTITY_ENV_ONLY=1`). It does not source `/root/FMP_SCREENER/.env`. Exit 2, 3, 4, or 5 fails deploy. Everyday deploy also runs `python -m qc_research.contracts.digests` before migrations.
