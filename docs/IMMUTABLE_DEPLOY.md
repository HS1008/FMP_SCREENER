...
`DASHBOARD_READONLY_URL` is required for the intended Streamlit identity. It is **not** `mi_readonly` (that role cannot SELECT `strategies` / `backtests`). Until the role is provisioned, Strategy Monitor falls back to the writer engine so current hosts keep rendering. Deploy runs `python -m jobs.verify_dashboard_readonly` after migrations (exit 2 fails deploy; exit 3 means unset / skip).
