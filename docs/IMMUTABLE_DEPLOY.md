...
| `/etc/fmp/secrets/dashboard_readonly.pw` | root 0600 | canonical Strategy Monitor secret for everyday deploy and MI activate |
| `/root/FMP_SCREENER/.secrets/dashboard_readonly.pw` | root 0600 | legacy only; one-way migrate into `/etc/fmp/secrets/`, never recreated |
...
Everyday deploy and `deploy_release.sh` run `scripts/provision_dashboard_readonly.sh --require` after migrations, then `scripts/verify_dashboard_identity.sh`. The password file at `/etc/fmp/secrets/dashboard_readonly.pw` is required. A leftover checkout copy at `/root/FMP_SCREENER/.secrets/dashboard_readonly.pw` is migrated once into `/etc/fmp/secrets/` and kept; deploy does not recreate checkout-local production secrets. The provision script applies `db/roles/dashboard_readonly.sql` via admin URL or local postgres peer (never the dashboard writer) and writes `DASHBOARD_READONLY_URL` into `/etc/fmp/fmp-dashboard.env` and `/root/FMP_SCREENER/.env` without printing values. After writing, it removes writer keys (`DATABASE_URL`, `DB_*`, `MARKET_INTELLIGENCE_DATABASE_URL`, `DASHBOARD_ALLOW_WRITER_FALLBACK`) from the systemd Streamlit env only; the checkout `.env` keeps writer keys for ingest/CLI. A missing password file fails deploy (exit 3). `DASHBOARD_ALLOW_WRITER_FALLBACK=1` fails deploy (exit 4). `STREAMLIT_ALLOW_PROVIDER_FETCH=1` fails deploy (exit 5). Neither escape is a production path.
...
Everyday activate of `/root/FMP_SCREENER` fetches the requested SHA from the authoritative repository URL with ancestry. It must not copy an unmarked shallow commit from the depth-1 staged release. After checkout, deploy fail-closes unless `HEAD` matches the requested SHA, `git log -1` succeeds, and every parent of `HEAD` exists.

A successful deploy on `main` then starts Stage 1, Platform Research, and Market Intelligence production verify. GitHub concurrency is per-workflow with `cancel-in-progress: false` so a sibling is not dropped. Stage 1 and Market Intelligence mutating host sessions share `/var/lock/fmp-post-deploy.lock`. Platform Research is read-only and does not take that lock. This is separate from `/var/lock/fmp-deploy.lock`.
