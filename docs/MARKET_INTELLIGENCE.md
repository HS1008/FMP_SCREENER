...
| AI gateway (REST `/api/v1` + MCP Streamable HTTP + stdio, OAuth 2.1/PKCE): semantic tools over the same `mi_v_*` views; remote sessions always `external` export policy, owner mode only for proven local sessions; holdout fail-closed views (migration 027) + gateway double filter; no arbitrary SQL, no execution, no model binaries | IMPLEMENTED_AND_TESTED (unit + PostgreSQL adversarial tests) | `ai_gateway/`, `db/migrations/027_ai_gateway_strategy_views.sql`, `docs/AI_GATEWAY.md` | same token + `DATABASE_READONLY_URL`; public HTTPS = CONFIGURATION_REQUIRED; remote value export of INTERNAL_ONLY/RESTRICTED sources = ENTITLEMENT_REQUIRED (`AI_GATEWAY_REMOTE_VALUE_SOURCES`) |
