...
| AI gateway (REST `/api/v1` + MCP Streamable HTTP + stdio, OAuth 2.1/PKCE): semantic tools over the same `mi_v_*` views; remote sessions always `external` export policy, owner mode only for proven local sessions; holdout fail-closed views (migrations 027-029) + gateway double filter; no arbitrary SQL, no execution, no model binaries | IMPLEMENTED_AND_TESTED (unit + PostgreSQL adversarial tests) | `ai_gateway/`, `db/migrations/027_ai_gateway_strategy_views.sql`, `db/migrations/028_gateway_holdout_failclosed.sql`, `db/migrations/029_holdout_accessed_nullable.sql`, `docs/AI_GATEWAY.md` | same token + `DATABASE_READONLY_URL`; public HTTPS = CONFIGURATION_REQUIRED; remote value export of INTERNAL_ONLY/RESTRICTED sources = ENTITLEMENT_REQUIRED (`AI_GATEWAY_REMOTE_VALUE_SOURCES`) |
...
| Streamlit pages 10-20 (Market Pulse, Macro, Rates, Credit, Sector Rotation V2, Data Health, Morning Context, PIT Sector Internals, Order Flow, Fixed Income, Commodities) | IMPLEMENTED_AND_TESTED (AppTest populated / empty / unconfigured) | `pages/1*.py`, `pages/20_Commodities.py`, `pages_ui.py` | `DATABASE_READONLY_URL`; no writer fallback |
...
| OpenBB / Cboe delayed options chains + VX_EOD (`OPENBB_CBOE_OPTIONS`, `OPENBB_CBOE_VIX`) | IMPLEMENTED_AND_TESTED (disposable PG + bounded live Cboe; production collection off; Cboe contractual gates pending) | `market_intelligence/openbb_provider/`, migration `033_openbb_options_vix.sql`, `docs/OPENBB_CBOE_RIGHTS.md` | Independent product gates: OPTIONS needs `MI_OPENBB_OPTIONS_RIGHTS_ACK` after Cboe website consent; VIX needs `MI_OPENBB_VIX_RIGHTS_ACK` after a CFE Data Agreement. Legacy `MI_OPENBB_CBOE_RIGHTS_ACK` is ignored. Data Health is `RIGHTS_PENDING` / `AGREEMENT_REQUIRED`, not FAILED. Export `INTERNAL_ONLY`. Not on remote AI allowlist |
| IBKR options chains (`IBKR_OPTIONS` prototype) | IMPLEMENTED_AND_TESTED (offline collector + live TWS geometry; API OPRA NBBO not proven) | `ibkr_collector/options.py`, `market_intelligence/ibkr_options.py`, `docs/IBKR_OPTIONS.md` | After paid OPRA L1 + TWS restart + Type 2 frozen: still 354 / no bid/ask. Data Health `PROVIDER_SUPPORT_REQUIRED`. Storage `RIGHTS_PENDING`. Collection off. INTERNAL_ONLY |
...
`CONFIGURED`, `CONFIGURATION_REQUIRED`, `DISABLED`, `ENTITLEMENT_REQUIRED`, `AGREEMENT_REQUIRED`,
`PROVIDER_SUPPORT_REQUIRED`, `RIGHTS_PENDING`, `NOT_CONFIGURED`, `ON_DEMAND`, `AVAILABLE`.
Observation freshness on Data Health is distinct from access: `LATEST_AVAILABLE`,
`CURRENT_TO_SOURCE` (collector matches provider; publication lag), `STALE_INGESTION`
(DB behind provider), `STALE_UPSTREAM` (provider has not printed newer).
OpenBB policy labels on Data Health: `RIGHTS_PENDING` (OPTIONS website consent), `AGREEMENT_REQUIRED` (CFE), `DISABLED` (rights recorded, collection off). A licensing gate is not `FAILED`.
...
* OpenBB / Cboe delayed options and VX_EOD (`INTERNAL_ONLY`): website delayed-quotes JSON and CFE
  VX_EOD, not OPRA, not official settlement. OPTIONS storage needs Cboe written consent
  (`MI_OPENBB_OPTIONS_RIGHTS_ACK`); VIX needs a CFE Data Agreement (`MI_OPENBB_VIX_RIGHTS_ACK`).
  Project governance is not that consent. Remote AI values stay redacted unless a human adds the
  source id to `AI_GATEWAY_REMOTE_VALUE_SOURCES` after those rights exist. Change is
  ENTITLEMENT_REQUIRED / AGREEMENT_REQUIRED. See `docs/OPENBB_CBOE_RIGHTS.md`.
...
* OpenBB / Cboe collection is implemented, fixture-tested, and query-backed on disposable
  PostgreSQL with a bounded live Cboe fetch. Production flags stay `0`. Product-specific rights
  acks stay unset until Cboe consent / CFE agreement exist. Deploy does **not**
