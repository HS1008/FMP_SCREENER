...
| IBKR options chains (`IBKR_OPTIONS` prototype) | IMPLEMENTED_AND_TESTED (offline collector + live TWS geometry; quotes not yet entitled; no production timer) | `ibkr_collector/options.py`, `market_intelligence/ibkr_options.py`, `docs/IBKR_OPTIONS.md` | Live 2026-09-14: `reqSecDefOptParams` + ATM `reqContractDetails` work; delayed SPY equity L1 works; delayed OPRA bid/ask did **not** reach the API (10167/10091, `quoted=0`). `MI_IBKR_OPTIONS_ENABLED=0`. Not in refresh. INTERNAL_ONLY |
