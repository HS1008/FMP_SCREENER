...
| Official EIA v2 energy (NG storage, petroleum inventories, US48 RTO demand/generation/interchange) | none / generated EIA wholesale samples (removed from production) | `EIA` adapter → `mi_eia_*` (migration 034). Flag `MI_EIA_ENABLED` distinct from `EIA_API_KEY` | FIXTURE_ONLY (pagination/incomplete extract/no-synthetic). Live catalog ingest CONFIGURATION_REQUIRED until key+flag | Native EIA cadence; revision overlap; HTTP 200 ≠ complete | `ATTRIBUTION_REQUIRED` | UNAVAILABLE where no exact source (hub power prices, NG/petroleum production routes unverified) | Provision key without enabling; verify routes live; do not relabel unverified cache as official |
| CFTC COT futures-only (TFF `gpe5-46if`, Disaggregated `72hh-3qpy`) | none | `CFTC_COT` → `mi_cot_*`. Combined reports catalogued, never ingested/summed | FIXTURE_ONLY (family split, missing/zero OI, delayed Friday publication) | Tuesday positions; normal Friday 15:30 ET is not historical proof | `ATTRIBUTION_REQUIRED` | none | `MI_COT_ENABLED` remains off |
| OpenFIGI identifier mapping | none | Event-driven `OPENFIGI` cache; ambiguous matches quarantined; transient HTTP not cached as NO_MATCH | FIXTURE_ONLY | Event-driven | `ATTRIBUTION_REQUIRED` | explicit unresolved mapping | Mapper only; not a universe |
| SEC filings / facts / metrics / events | FMP fundamentals/ratios/calendar | `SEC_EDGAR` → `mi_sec_*` (migration 035). Rule-based events. Metrics refuse mixed YTD and invalid denominators | FIXTURE_ONLY | Bounded issuers; fair-access User-Agent | `ATTRIBUTION_REQUIRED` | FMP calendar/news/estimates remain explicit transition | `MI_EDGAR_ENABLED` off; do not invent custom concepts |
| Option exposure magnitude / futures curves / FX mid | none | Versioned derived methods (`exposure_magnitude_v1`, `FUTURES_FRONT_DEFERRED_V1`, `FX_MID_V1`) | FIXTURE_ONLY | inherits option/futures/FX snapshots | `INTERNAL_ONLY` | INCOMPLETE/UNAVAILABLE when inputs fail | Not measured dealer GEX; not yield conversion |
