...
Cache keys are partitioned by the *effective export mode* of the calling session so a
payload built for a local owner session can never be served to a remote client.
"""
...
def _partition() -> str:
    ...
    ctx = current_context()
    mode = effective_export_mode(ctx)
    sources = ",".join(effective_remote_value_sources(ctx)) if mode == "external" else ""
    return "{0}|{1}".format(mode, sources)


def cached(name: str, key: str, builder: Callable[[], T]) -> T:
    cache_key = "{0}:{1}:{2}".format(_partition(), name, key)
