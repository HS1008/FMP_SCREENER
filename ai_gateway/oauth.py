...
async def token_endpoint(request: Request) -> JSONResponse:
    ...
    if not verifier or _s256(verifier) != record["code_challenge"]:
        return JSONResponse({"error": "invalid_grant"}, status_code=400)
    audience = resource or record["resource"] or resource_url(request)
    access = mint_access_token(audience=audience, ttl_seconds=TOKEN_TTL_SECONDS, scope=record["scope"])
