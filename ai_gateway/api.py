...
from ai_gateway.config import MAX_REQUEST_BYTES, cors_origins, public_base_url
from ai_gateway.envelope import public_payload
from ai_gateway.errors import GatewayError, PAYLOAD_TOO_LARGE, UNAUTHORIZED
from ai_gateway.logging import fingerprint, new_request_id, timed_call
...
def require_auth(request: Request) -> dict[str, Any]:
    ...
    identity = authenticate(request.headers.get("authorization"), audience=audience + "/mcp")
    # Also accept tokens minted for the API origin itself.
    return identity


def _client_identity(request: Request, auth: dict[str, Any]) -> str:
    return auth.get("token_fp") or request.client.host if request.client else "unknown"
...
def _call(tool: str, request: Request, auth: dict[str, Any], arguments: dict[str, Any] | None = None) -> JSONResponse:
    ...
    with timed_call("gateway_tool", tool=tool, request_id=request_id, auth=auth.get("method")) as extra:
...
def list_tools(request: Request, auth: dict[str, Any] = Depends(require_auth)) -> dict[str, Any]:
    enforce(_client_identity(request, auth))
    return {"server": SERVER_NAME, "schema_version": SCHEMA_VERSION, "tools": tools_list()}
...
def openapi_document(request: Request) -> dict[str, Any]:
    ...
    paths = {
        ...
        "/api/v1/rates": {"get": {"summary": "Rates curve"}},
        ...
        "/api/v1/sectors": {"get": {"summary": "Sector rotation"}},
        "/api/v1/sectors/{sector}": {"get": {"summary": "Sector detail"}},
        "/api/v1/industries": {"get": {"summary": "Industry rotation"}},
        "/api/v1/subindustries": {"get": {"summary": "Subindustry rotation (canonical leaf is industry)"}},
        "/api/v1/order-flow": {"get": {"summary": "FINRA TRACE aggregates"}},
        "/api/v1/strategies": {"get": {"summary": "Strategy summaries"}},
        "/api/v1/strategies/{strategy}": {"get": {"summary": "One strategy summary"}},
        "/api/v1/data-health": {"get": {"summary": "Data freshness"}},
        "/api/v1/changes": {"get": {"summary": "What changed"}},
    }
    return {
        "openapi": "3.1.0",
        "info": {"title": SERVER_NAME, "version": SCHEMA_VERSION, "description": "Read-only Market Intelligence API."},
        "servers": [{"url": base}],
        "paths": paths,
...
def ready_alias(request: Request, auth: dict[str, Any] = Depends(require_auth)) -> JSONResponse:
    ...
    body = {"api": "v1", "database": probe, "token_configured": True, "ready": probe.get("status") == "OK", "gateway": SCHEMA_VERSION}
...
async def mcp_endpoint(request: Request) -> JSONResponse:
    ...
    for message in messages:
        ...
        with timed_call("mcp", tool=method, request_id=request_id, auth=auth.get("method")) as extra:
