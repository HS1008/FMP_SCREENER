...
def test_payload_keeps_explicit_tenor_order_and_tickers():
    ...
    assert payload["axis"] == ["1D", "9D", "1M", "3M", "6M", "1Y"]
    ...
    assert by_tenor["1D"]["value"] == 11.95
...
def test_missing_values_stay_missing_and_duplicates_keep_the_first_row():
    rows = [
        {"tenor": "1D", "yahoo_ticker": "^VIX1D", "value": None, "as_of": "2026-09-25"},
        {"tenor": "1D", "yahoo_ticker": "^VIX1D", "value": 0, "as_of": "2026-09-25"},
        {"tenor": "9D", "yahoo_ticker": "^VIX9D", "value": float("nan")},
        {"tenor": "1M", "yahoo_ticker": "^VIX", "value": float("inf")},
        {"tenor": "3M", "yahoo_ticker": "^VIX3M", "value": True},
    ...
    by_tenor = {point["tenor"]: point for point in payload["points"]}
    assert by_tenor["1D"]["value"] is None
    assert by_tenor["9D"]["value"] is None
...
def test_echarts_option_is_a_categorical_curve_without_zoom():
    ...
    assert option["xAxis"]["data"] == ["1D", "9D", "1M", "3M", "6M", "1Y"]
