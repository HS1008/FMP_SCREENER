...
def test_gateway_migration_is_the_next_number_and_last():
    ...
    assert names[-1] == "034_provider_neutral_options_delay.sql"
...
def test_scenario_a_fresh_database_applies_everything_once_with_hashes(scratch_db):
    ...
    assert first[-1] == "034_provider_neutral_options_delay.sql"
...
def test_scenario_b_simulated_production_001_019_upgrades_through_028(scratch_db):
    ...
    assert applied[-1] == "034_provider_neutral_options_delay.sql"
    recorded = _recorded(engine)
