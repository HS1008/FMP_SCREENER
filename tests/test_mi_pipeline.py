...
MI_PAGES = sorted(
    ...
    if p.stem[:2].isdigit() and int(p.stem.split("_", 1)[0]) >= 10
)
...
def test_refresh_dry_run_makes_no_calls_and_no_writes(pg_engine, populated, capsys):
    ...
    assert [s["step"] for s in out["plan"]["steps"]] == ["fred", "finra", "treasury", "equity", "options", "vix", "cboe", "cftc", "eia", "openfigi", "build_analytics", "build_morning"]
