...
def test_options_page_is_read_only_source():
    page = (ROOT / "pages" / "20_Options.py").read_text(encoding="utf-8")
    ui = (ROOT / "market_intelligence" / "pages_ui.py").read_text(encoding="utf-8")
    render = ui.split("def render_options()", 1)[1].split("\ndef render_market_hub", 1)[0]
    for blob in (page, render):
        assert "cboe_client" not in blob
        assert "urllib" not in blob
        assert "http" not in blob.lower() or "https" not in blob
        assert "INSERT " not in blob and "UPDATE " not in blob
        assert "execute(" not in blob
    assert "load_or_stop" in render
...
def test_options_page_renders_null_as_blank(monkeypatch):
    ...
    monkeypatch.setattr("market_intelligence.pages_ui.load_or_stop", lambda *args, **kwargs: ctx)
    at = AppTest.from_file(str(ROOT / "pages" / "20_Options.py"), default_timeout=30)
    at.run()
    assert not at.exception, [e.value for e in at.exception]
    text = " ".join(str(item.value) for item in list(at.metric) + list(at.info) + list(at.caption) + list(at.warning))
    assert "—" in text or "unavailable" in text.lower()
    assert "0.00" not in text
    assert "not a live quote" in text.lower() or "not a live" in text.lower()
