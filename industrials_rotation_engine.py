"""
Industrials industry rotation vs XLI (relative strength heatmap).

Each row is an **equal-weight basket** of listed proxies (stocks or ETFs), rebased to 1 on the
first aligned date, then averaged; RS vs XLI is that composite divided by XLI rebased the same way.
Uses dividend-adjusted prices from ``data_loader.get_price_histories_long``.

Breadth proxy row uses **RSPN** (Invesco S&P 500 Equal Weight Industrials ETF; post-2023 ticker).
"""
...
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("Aerospace & Defense", ("GE", "RTX", "LMT", "NOC", "GD")),
    ("Railroads", ("UNP", "CSX", "NSC", "CNI")),
    ("Logistics & Parcel Delivery", ("UPS", "FDX", "EXPD")),
    ("Industrial Machinery & Heavy Equipment", ("CAT", "DE", "PCAR", "CMI")),
    ("AI Power & Electrical Infrastructure", ("ETN", "PH", "HUBB", "VRT", "NVT")),
    ("Engineering & Construction", ("PWR", "FIX", "FLR", "ACM")),
    ("Building Products & HVAC", ("TT", "CARR", "JCI", "LII")),
    ("Factory Automation & Robotics", ("ROK", "ABB", "FANUY", "EMR")),
    ("Waste & Environmental Services", ("WM", "RSG", "CWST")),
    ("Airlines & Air Travel", ("DAL", "UAL", "LUV", "AAL")),
    ("Staffing & Labor Services", ("MAN", "RHI", "KFY")),
    ("Industrial Rentals & Services", ("URI", "HRI", "GWW")),
    ("Marine Shipping & Transport", ("MATX", "ZIM", "SBLK")),
    ("Agriculture Equipment", ("DE", "AGCO", "CNH")),
    ("Industrials Breadth Proxy", ("RSPN",)),
)
...
def get_industrials_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLI and all group proxy symbols."""
    end = date.today()
...
def build_industrials_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    try:
        prices = get_industrials_rotation_prices(session, api_key, force_refresh=force_refresh)
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLI or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLI).",
