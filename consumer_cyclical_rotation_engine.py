"""
Consumer Cyclical industry rotation vs XLY (relative strength heatmap).

Each row is an **equal-weight basket** of listed proxies (stocks or ETFs), rebased to 1 on the
first aligned date, then averaged; RS vs XLY is that composite divided by XLY rebased the same way.
...
# (display name, tuple of FMP symbols — equal-weight composite vs XLY)
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("E-Commerce & Internet Retail", ("AMZN", "MELI", "ETSY")),
    ("Home Improvement & Housing Retail", ("HD", "LOW", "RH")),
    ("Auto Manufacturers", ("TSLA", "GM", "F")),
    ("Auto Parts & Service", ("ORLY", "AZO", "AAP")),
    ("Travel & Leisure", ("BKNG", "ABNB", "EXPE")),
    ("Restaurants & Fast Casual", ("MCD", "CMG", "SBUX", "YUM")),
    ("Luxury & Premium Brands", ("LVMUY", "RACE", "CPRI")),
    ("Apparel & Footwear", ("NKE", "DECK", "CROX", "ONON")),
    ("Casinos & Gaming", ("LVS", "WYNN", "MGM", "CZR")),
    ("Cruise Lines & Vacation", ("RCL", "CCL", "NCLH")),
    ("Off-Price & Discount Retail", ("TJX", "ROST", "BURL")),
    ("Consumer Breadth Proxy", ("XRT",)),
)
...
def get_consumer_cyclical_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLY and all group proxy symbols."""
    end = date.today()
...
def build_consumer_cyclical_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    try:
        prices = get_consumer_cyclical_rotation_prices(session, api_key, force_refresh=force_refresh)
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLY or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLY).",
