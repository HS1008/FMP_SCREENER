"""
Healthcare industry rotation vs XLV (relative strength heatmap).

Each row is an **equal-weight basket** of listed proxies (stocks or ETFs), rebased to 1 on the
first aligned date, then averaged; RS vs XLV is that composite divided by XLV rebased the same way.
...
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("Large Pharma", ("LLY", "JNJ", "MRK", "PFE")),
    ("Biotechnology", ("REGN", "VRTX", "BIIB", "GILD")),
    ("Obesity & Metabolic Drugs", ("LLY", "NVO")),
    ("Medical Devices", ("ISRG", "SYK", "MDT", "BSX")),
    ("Healthcare Equipment & Supplies", ("BDX", "DXCM", "ABT")),
    ("Managed Care / Insurance", ("UNH", "HUM", "CI", "ELV")),
    ("Healthcare Services", ("HCA", "THC", "UHS")),
    ("Diagnostics & Life Science Tools", ("TMO", "DHR", "IQV")),
    ("CROs & Drug Development", ("IQV", "MEDP")),
    ("Genomics & Precision Medicine", ("PACB", "CRSP", "BEAM")),
    ("Equal-Weight Healthcare Breadth", ("XBI",)),
)
...
def get_healthcare_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLV and all group proxy symbols."""
    end = date.today()
...
def build_healthcare_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    try:
        prices = get_healthcare_rotation_prices(session, api_key, force_refresh=force_refresh)
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLV or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLV).",
