"""
Financial Services industry rotation vs XLF (relative strength heatmap).

Each row is an **equal-weight basket** of listed proxies (stocks or ETFs), rebased to 1 on the
first aligned date, then averaged; RS vs XLF is that composite divided by XLF rebased the same way.
...
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("Regional Banks", ("MTB", "FITB", "RF", "KEY")),
    ("Investment Banks & Capital Markets", ("GS", "MS", "SCHW")),
    ("Asset Managers & Alternatives", ("BLK", "BX", "APO", "KKR")),
    ("Exchanges & Market Infrastructure", ("CME", "ICE", "SPGI", "MCO")),
    ("Credit Cards & Payment Networks", ("V", "MA", "AXP", "COF")),
    ("Fintech & Digital Payments", ("PYPL", "SQ", "SOFI")),
    ("Insurance", ("PGR", "CB", "ALL", "TRV")),
    ("Mortgage & Housing Finance", ("RKT", "UWMC")),
    ("Consumer Finance", ("SYF", "DFS", "COF")),
    ("Equal-Weight Financial Breadth", ("KBE",)),
)
...
def get_financial_services_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLF and all group proxy symbols."""
    end = date.today()
...
def build_financial_services_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    try:
        prices = get_financial_services_rotation_prices(session, api_key, force_refresh=force_refresh)
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLF or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLF).",
