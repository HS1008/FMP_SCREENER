"""
Real Estate industry rotation vs XLRE (relative strength heatmap).

Each row is an **equal-weight basket** of listed REIT proxies, rebased to 1 on the
first aligned date, then averaged; RS vs XLRE is that composite divided by XLRE rebased the same way.
Uses dividend-adjusted prices from ``data_loader.get_price_histories_long``.

Breadth proxy row uses **VNQ** (Vanguard Real Estate ETF).
"""
...
INDUSTRY_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("Cell Tower REITs", ("AMT", "CCI", "SBAC")),
    ("Industrial & Logistics REITs", ("PLD", "FR", "TRNO")),
    ("Residential REITs", ("AVB", "EQR", "ESS")),
    ("Single Family Rental REITs", ("INVH", "AMH")),
    ("Retail REITs", ("SPG", "MAC", "REG")),
    ("Net Lease REITs", ("O", "NNN", "WPC")),
    ("Office REITs", ("BXP", "VNO", "KRC")),
    ("Hotel & Hospitality REITs", ("HST", "RHP", "PK")),
    ("Healthcare REITs", ("WELL", "VTR", "OHI")),
    ("Self Storage REITs", ("PSA", "EXR", "CUBE")),
    ("Mortgage REITs", ("NLY", "STWD", "BXMT")),
    ("Real Estate Breadth Proxy", ("VNQ",)),
)
...
def get_real_estate_rotation_prices(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
) -> pd.DataFrame:
    """Long-format dividend-adjusted closes for XLRE and all group proxy symbols."""
    end = date.today()
...
def build_real_estate_rotation_bundle(
    session: Any,
    api_key: str,
    *,
    force_refresh: bool = False,
    ...
    if prices.empty or BENCHMARK not in prices["symbol"].unique():
        return {
            "ok": False,
            "error": "Missing price history for XLRE or rotation symbols.",
            "as_of": None,
            "prices": prices,
            "metrics": pd.DataFrame(),
            "heatmap": pd.DataFrame(),
    ...
    if metrics.empty:
        return {
            "ok": False,
            "error": "Could not compute RS metrics (insufficient overlapping history vs XLRE).",
